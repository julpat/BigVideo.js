...
(function (factory) {
	...
	$.BigVideo = function(options) {
		...
		function updateSize() {
			...
			if (containerAspect < mediaAspect) {
				...
				if (currMediaType == 'video') {
					...
					if (!settings.shrinkable) {
						$(vidEl).css('height',containerH);
							top = 0;
							left = -(containerH*mediaAspect-containerW)/2;
					} else {
						$(vidEl).css('height',containerW/mediaAspect);
			...
			$(vidEl).css({
				'top': settings.alignVertical === 'top' ? 0 : settings.alignVertical === 'bottom' ? 'auto' : top,
				'bottom': settings.alignVertical === 'bottom' ? 0 : bottom,
				'left': settings.alignHorizontal === 'left' ? 0 : settings.alignHorizontal === 'right' ? 'auto' : left,
				'right': settings.alignHorizontal === 'right' ? 0 : right
			});
		}
